...
class YSharedPreferences {
  YSharedPreferences._(this._preferenceCache);

  static const String _prefix = 'flutter.';
  static Completer<YSharedPreferences>? _completer;
  ...
  /// Loads and parses the [YSharedPreferences] for this app from disk.
  ///
  /// Because this is reading from disk, it shouldn't be awaited in
  /// performance-sensitive blocks.
  static Future<YSharedPreferences> getInstance() async {
    if (_completer == null) {
      final completer = Completer<YSharedPreferences>();
      try {
        final Map<String, Object> preferencesMap =
            await _getSharedPreferencesMap();
        completer.complete(YSharedPreferences._(preferencesMap));
      } on Exception catch (e) {
        // If there's an error, explicitly return the future with an error.
        // then set the completer to null so we can retry.
        completer.completeError(e);
        final Future<YSharedPreferences> sharedPrefsFuture = completer.future;
        _completer = null;
  ...
  ///
  /// It is instantiated to the current state of the YSharedPreferences or
  ...
  Future<void> reload() async {
    final Map<String, Object> preferences =
        await YSharedPreferences._getSharedPreferencesMap();
    _preferenceCache.clear();
